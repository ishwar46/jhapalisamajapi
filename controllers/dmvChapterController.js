...
exports.getDMVChapterPage = async (req, res) => {
    try {
        let page = await DMVChapterPage.findOne().populate("members.user", "fullName profilePicture email");
        if (!page) {
            page = new DMVChapterPage({
                pageTitle: "Washington DC, Virginia & Maryland Chapter",
                pageDescription: "Meet our DMV Chapter members who lead our regional initiatives."
            });
            await page.save();
            page = await DMVChapterPage.findOne().populate("members.user", "fullName profilePicture email");
        }
        return res.status(200).json(page);
    } catch (error) {
        console.error("getDMVChapterPage Error:", error);
        return res.status(500).json({ error: "Server error fetching DMV Chapter page." });
    }
};
...
exports.updateDMVChapterPage = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Server error updating DMV Chapter page." });
    }
};
...
exports.addDMVMember = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Server error adding DMV Chapter member." });
    }
};
...
exports.updateDMVMember = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Server error updating DMV Chapter member." });
    }
};
...
exports.deleteDMVMember = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Server error deleting DMV Chapter member." });
    }
};
